...
# from azure import azure_upload_file, azure_download_file
...
connect_db()
UPLOAD_FOLDER = 'buffer/upload_buffer/'
...
def download_file():
    if(request.method == 'POST'):
        ...
        if response != None:
           ...
           return send_from_directory(directory='buffer/download_buffer/', filename=response.file_name)
        else:
